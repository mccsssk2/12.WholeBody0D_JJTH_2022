...
// %
// JJ TH NOV 3
double sigma_lv     				= data->p_ursino[113]; // % units: mmHg/ml
double sigma_rv     				= data->p_ursino[114]; //  units: mmHg/ml
...
double sigma_R 							= data->p_ursino[116]; //  units: mmHg/(ml/s)
...
/************************ VENA CAVA ************************************/
if(Y[13] > Y[35])	{
  data->Qsup    = (Y[13] - Y[35])/data->p_R[1];
}else {
  data->Qsup    = 0;
}

data->Qab	      = (Y[8-1] - Y[15-1])/data->p_R[2]	;

if(Y[14] > Y[35])	{
  data->Qinf	  = (Y[14] - Y[35])/data->p_R[3];
}	else {
  data->Qinf	  = 0;
}
...
if(Y[35] > Y[15]){
  data->Qrao = (Y[35] - Y[15])/data->p_R[4];
}	else{
  data->Qrao = 0;
}
...
Y[11] is P_lv. */
if(Y[34] > Y[11]) {
	data->Qlao = (Y[34] - Y[11]) / data->p_R[8];
}else {
  data->Qlao = 0.0;
}
...
data->Qupi	= (Y[50] - Y[3])	/(data->p_R[13]* sigma_R)	;

if (Y[3] > Y[13]){
	data->Qupo	 = (Y[3]   - Y[13])	/data->p_R[14]	;
}else{
	data->Qupo= 0;
}
...
Y[7] is P_ab
*/
data->Qsp1 = (Y[52] - Y[5])	/(data->p_R[15] * sigma_R)	;

if(Y[8-1] < Y[6-1]){
  data->Qsp2 = (Y[6-1]   - Y[8-1])	/data->p_R[16]	;
}else{
  data->Qsp2 = 0;
}
...
data->QkRi = (Y[52] - Y[48])/data->p_R[17]; // right kidney inlet flow.
data->QkLi = (Y[52] - Y[49])/data->p_R[18]; // left kidney inlet flow.

data->Qk1 					=  data->QkRi + data->QkLi;
data->Qk2           = 0.0;
for(i = 0; i<12; i++){
  data->Q_kidneys[i]      = (Y[48] - Y[36+i])/(data->p_R[19+i]* sigma_R); // Qki1 to Qk12
  data->Q_kidneys[12 + i] = (Y[36+i] - Y[7])/(data->p_R[31+i]* sigma_R);  // Qko1 to Qko12
...
Y[6] is P_ll,
Y[7] is P_ab.  */
data->Qll1	 	= (Y[52] - Y[6])	/(data->p_R[43]* sigma_R);

if (Y[6] > Y[7]){
	data->Qll2	= (Y[6]   - Y[7])	/data->p_R[44]	;
}else{
	data->Qll2	= 0;
}
...
if(data->DialysisOnOff == 1){
  dY[2] 	= ( -(Fa - Rv) - QF + Qinfused)/1000.; 	// % eq. 3, appedix 1, Vpl. Vpl is plasma volume. Ursino 2000. unit: L.
}else if(data->DialysisOnOff == 0){
  dY[2] 	= (-(Fa - Rv))/1000.; 	// % eq. 3, appedix 1, Vpl. Vpl is plasma volume. Ursino 2000. unit: L.
}else{
  printf("Dialysis On Off argv[3]=%d is not valid.",data->DialysisOnOff);
  exit(-1);
}

/* Y[3] is Pup, pressure in upper body, head and neck.
 		eq. 22, appendix 2, p585.*/
dY[3]     = (data->Qupi - data->Qupo + (V_gain*ZPFV_up)) / data->p_C[10];

/*************** detailed kidney ************************************/
// original formula.
dY[4]     = 0.0; // (Qk1 - Qk2) / Ck; // % Y[5-1] is Pk, Ck is non-zero. eq. 23, appendix 2, p585.

// broken down to 12 eqns.
for(i = 0; i<12; i++){
  dY[36+i] = (data->Q_kidneys[i] - data->Q_kidneys[12 + i] + (V_gain*ZPFV_kid/12))/(data->p_C[14+i]);
}
...
dY[5]     = (data->Qsp1 - data->Qsp2 + (V_gain * ZPFV_sp)) / data->p_C[11];

// % Y[6] is Pll, legs pressure.
dY[6]     = (data->Qll1 - data->Qll2 + (V_gain * ZPFV_ll)) / data->p_C[26];

// % Y[7] is Pab.
dY[7]     = (data->Qk2 + data->Qsp2 + data->Qll2 - data->Qab + (V_gain * ZPFV_ab)) / data->p_C[2] ;

// % Y[8] is Pth: thoracic pressure. aka Y8 is Pbias. p1241, Heldt 2002 paper.
dY[8]     = (2*M_PI*respRate)*cos(2*M_PI*respRate*data->p_ursino[1]); // % This varies with respiratory variation, mean value taken from Heldt thesis 2.3 p.47 resprate units are (breath/s).

/***************************** ODEs for Compliances **************************/
dY[9] = (Clv - Y[9])/DELTAT;    // Left Ventricle Compliance
dY[10] = (Crv - Y[10])/DELTAT;  // Right Ventricle Compliance
dY[32] = (Cla - Y[32])/DELTAT;  // Left Atrial Compliance
dY[33] = (Cra - Y[33])/DELTAT;  // Right Atrial Compliance

/****************************************************************************************************************************/
// % Y[11] is LV pressure.
dY[11]    = (1.0 / Y[9])  * ((Y[8] - Y[11]) * dY[9] + data->Qlao - data->Qlo ) + dY[8];

// Y34 is left atrial pressure.
dY[34]    = (1.0 / Y[32]) * ((Y[8] - Y[34]) * dY[32] + data->Qli - data->Qlao) + dY[8];

// Y[15] is Right Ventricle Pressure
dY[15]    = (1.0 / Y[10]) * ((Y[8] - Y[15]) * dY[10] + data->Qrao - data->Qro) + dY[8];

// Y[35] is right atrial pressure
dY[35]    = (1.0 / Y[33]) * ((Y[8] - Y[35]) * dY[33] + data->Qinf + data->Qsup - data->Qrao) + dY[8];

// % Y[12] is Pa, ascending aorta pressure.
if(data->DialysisOnOff == 1){
  dY[12]    = (1.0/data->p_C[6]) * (data->Qlo + ( -(Fa - Rv) - QF + Qinfused) - (data->Q_ao_bra + data->Q_ao_tho)) + 0.333*dY[8];
}else if(data->DialysisOnOff == 0){
  dY[12]    = (1.0/data->p_C[6]) * (data->Qlo -(Fa - Rv) - (data->Q_ao_bra + data->Q_ao_tho)) + 0.333*dY[8];
}else{
  printf("Dialysis On Off argv[3]=%d is not valid.",data->DialysisOnOff);
  exit(-1);
}
...
dY[13]    = (data->Qupo - data->Qsup + V_gain * ZPFV_sup) / data->p_C[1] + dY[8];
// % Y[14] is Pinf, eq. 28 appendix 2.
dY[14]    = (data->Qab - data->Qinf + V_gain * ZPFV_inf) / data->p_C[3] + dY[8];

// % Y[16] is Ppa. Pulmonary circulation inlet pressure.
dY[16]    = (data->Qro - data->Qpa) / data->p_C[4] + dY[8]; // % Y[17-1] is Ppa, eq. 30 appendix 2.

// % Y[17] is Ppv. Left ventricle inlet pressure.
dY[17]    = (data->Qpa - data->Qli) / data->p_C[5] + dY[8]; // % Y[18-1] is Ppv, eq. 31 appendix 2.

//  Brachiocephalic Aortic Pressure
dY[50] = (data->Q_ao_bra - data->Qupi)/data->p_C[7] + dY[8]; //  Brachiocephalic Aortic Pressure
...
dY[26-1]   = -phi_u 	  - J_u/1000. 		+ (Qinfused*(double)(data->DialysisOnOff)/1000.)*cuinf; // % urea eq 14, p207, Ursino 2000. combined with eqn 27, Ursino 2008.
dY[27-1]   = -phi_na    - J_na/1000.		+ (Qinfused*(double)(data->DialysisOnOff)/1000.)*cnainf; // % sodium 	eq 14, p207, Ursino 2000.combined with eqn 27, Ursino 2008.
dY[28-1]   = -phi_k 	  - J_k/1000. 		+ (Qinfused*(double)(data->DialysisOnOff)/1000.)*ckinf; // % potassium  eq 14, p207, Ursino 2000.combined with eqn 27, Ursino 2008.
dY[29-1]   = -phi_cl 	  - J_cl/1000.    + (Qinfused*(double)(data->DialysisOnOff)/1000.)*cclinf; // % chlorine 	eq 14, p207, Ursino 2000.
