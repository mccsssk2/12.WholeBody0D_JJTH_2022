...
The idea is to get this solution with forward sensitivity without the jacobian.
In the process, learn summat about CVODES and the FSA functions.
...
//  if (sensi) { // always do sensitivity

// this is how you say which parameters are for sensivity. so pbar can be a subset of data->p
    pbar[0] = data->p[0];
    pbar[1] = data->p[1];
    pbar[2] = data->p[2];

// whats this?
    yS = N_VCloneVectorArray_Serial(NS, y); // NS is 3, the number of parameters in the SA. So this is producing an array of NS size
    for (is=0;is<NS;is++) N_VConst(ZERO, yS[is]); // 

// Memory for sensitivity calculations and spefication of sensi_meth.
// cvode_mem is mem, NS is the number of parameters, fS is the function pointer to sensitivity RHS, and yS is the sensitivity vector.
    CVodeSensInit1(cvode_mem, NS, sensi_meth, fS, yS);

    CVodeSensEEtolerances(cvode_mem);
    CVodeSetSensErrCon(cvode_mem, err_con);
    CVodeSetSensParams(cvode_mem, NULL, pbar, NULL);
...
// time loop.
  for (iout=1, tout=T1; iout <= NOUT; iout++, tout *= TMULT) {
    CVode(cvode_mem, tout, y, &t, CV_NORMAL); // so in this case, the thing is solving the problem from the beginning. tout is just a number, not an array
